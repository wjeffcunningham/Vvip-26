...
import {
  AwsClient,
} from "https://esm.sh/aws4fetch@1.0.17";
...
serve(async (req) => {
  ...
  try {
    ...
    const r2Url = `${endpoint}/${bucket}/shoppe/shoppe.json`;
    ...
    if (!r2Res.ok) {
      const msg = await r2Res.text();
      console.error("R2 write failed:", r2Res.status, msg);
      return json({ error: "R2 write failed: " + msg }, 502);
    }

    return json({ ok: true, count: items.length });
  } catch (e) {
    console.error("update-shoppe error:", e);
